...
class ChatBot():
    ...
    def get_weather(self):
        ...
        try:
            rqst = get("http://api.openweathermap.org/data/2.5/find",
                       params={'q': city, 'type': 'like', 'units': 'metric', 'APPID': self.openweatherkey})
            ...
            weather = f"Сейчас в городе Санкт-Петербург температура равна {temperature_real} градусов (ощущается как {temperature_feel}). Давление равно " \
                      f"{pressure}{chr(160)}Па. Влажность равна{chr(160)}{humidity}%. Скорость ветра превышает{chr(160)}{wind_speed}{chr(160)}м/с. {rain}."
            return weather
        except:
    ...
    def get_info(self, team):
        ...
        try:
            ...
            while self.football_table[position]["team"] != team:
                position += 1
             f"Команда {self.football_table[position]['team']}" \
    ...
    def get_answer(self, text):
        ...
        if "time" in keys:
            ...
            return "Я могу рассказать про чемпионаты Испании, " \
                   ...
                   "Нидерландов (Голландии), России, Турции. Выберите страну, про чемпионат которой Вы бы хотели узнать."
